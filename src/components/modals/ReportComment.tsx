...
const ReportComment = () => {
    ...
    const { activeComment_id, setAll, showReportComment } = useModalState((state) => state);
    const theme = useTheme<Theme>();
  ...
  return (
    <ReactNavtieModalWrapper isVisible={showReportComment} height={ (HEIGHT / 100 * 60)} backgroundColor={theme.colors.secondaryBackGroundColor} >
        <Box width='100%' padding='m' position='relative'>
            ...
            <Box flexDirection='row' width='100%' justifyContent='space-between' alignItems='center' position='relative' zIndex={2}>
                ...
                <Feather name='x' onPress={() => setAll({ showReportComment: false })} color={theme.colors.textColor} size={30} />
            </Box>

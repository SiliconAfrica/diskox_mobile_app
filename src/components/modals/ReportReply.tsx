...
const ReportReply = () => {
    ...
    const { activeReply_id, setAll, showReportReply } = useModalState((state) => state);
    const theme = useTheme<Theme>();
    ...
    const { mutate, isLoading } = useMutation({
        mutationFn: (data: FormData) => httpService.post(`${URLS.REPORT_COMMENT}`, data),
        onSuccess: (data) => {
  ...
  return (
    <ReactNavtieModalWrapper isVisible={showReportReply} height={ (HEIGHT / 100 * 60)} backgroundColor={theme.colors.secondaryBackGroundColor} >
        <Box width='100%' padding='m' position='relative'>
            ...
            <Box flexDirection='row' width='100%' justifyContent='space-between' alignItems='center' position='relative' zIndex={2}>
                ...
                <Feather name='x' onPress={() => setAll({ showReportReply: false })} color={theme.colors.textColor} size={30} />
            </Box>

            <CustomText marginTop='s'>If this comment has violated any of our community guidelines, let us know what happened and we will look into it.</CustomText>

...
const ReportUser = () => {
    ...
    const { showReportPost, activePost, setAll, report_user_id } = useModalState((state) => state);
    const theme = useTheme<Theme>();
    ...
    const report = () => {
        if (activeReport === '' || report_user_id === null) {
            toast.show('You have to select an option', { type: 'warning' })
        }
        const formData = new FormData();
        formData.append('user_id', report_user_id.toString());

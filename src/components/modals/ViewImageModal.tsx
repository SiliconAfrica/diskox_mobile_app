    ...
    <ReactNavtieModalWrapper isVisible={isVisisble} height={'40%'} backgroundColor={theme.colors.secondaryBackGroundColor}>
        <Box width='100%' paddingHorizontal='m'>
            ...
            <ScrollView horizontal contentContainerStyle={{ paddingHorizontal: 20 }}>
                {
                    images.map((item, i) => (
                        <Box width={width} height={200} overflow='hidden' key={i.toString()} borderRadius={10} marginRight='m'>
                            <Image key={i.toString()} source={{ uri: `${IMAGE_BASE}${item.image_path}` }} contentFit='cover' style={{
                            width: width,
                            height: 200,
                            marginRight: 20,
                        }} />

...
const RepostFeedCard = ({ post, vote, handleNavigate, showReactions, handleBookmark, handleFollow, followLoading }: IProps) => {
    ...
    return (
        <Box width={'100%'} padding='m' borderRadius={10} borderWidth={1} borderColor='borderColor'>
                    ...
                    <Box width={'100%'} flexWrap='wrap' flexDirection='row' alignItems='center'>
                        <CustomText marginRight='s' variant='subheader' fontSize={14} >{name} @{username}
                          {post.community_id === null && post.tags.length===1 && (
                            <>
                              tagged  <CustomText variant='subheader' fontSize={14}>{post.tags[0].user.name } </CustomText>
                            </>
                          )}
                          {post.community_id === null && post.tags.length > 1 && (
                            <>
                             <CustomText variant='xs'> tagged </CustomText> <CustomText variant='subheader' fontSize={14}>{post.tags[0].user.username} and {post.tags.length - 1} others</CustomText>
                            </>
                          )}
                          ...
                          { myId !== userId && (
                              <CustomButton title={post.user.isFollowing === 1 ? 'Following' : 'Follow'} isLoading={followLoading} height={22} width={65} spinnerColor={theme.colors.textColor} onPress={handleFollow} color={isDarkMode ? 'black':theme.colors.fadedButtonBgColor} textColor={isDarkMode ? 'white':theme.colors.primaryColor} variant='subheader' />
                          )}
                    </Box>
                    <CustomText variant='body' fontSize={12} color='lightGrey'>{moment(post.created_at).fromNow()}</CustomText>
                </Box>
           </Box>

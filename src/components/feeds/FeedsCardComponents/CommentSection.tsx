import { View, Text, ScrollView, ActivityIndicator, NativeSyntheticEvent, TextInputKeyPressEventData } from 'react-native'
...
const CommentSection = ({
    ...
    const paginationMutation = useMutation({
      mutationFn: (data: string) => httpService.get(`${URLS.GET_COMMENTS_BY_POST_ID}/${postId}`, {
        params: {
          page: currentPage
        }
      }),
            ...
            if (item.code === CUSTOM_STATUS_CODE.SUCCESS) {
              if (comments.length > 0) {
                if (item.data?.data.length !== 0) {
                  ...
                  const uniqArr = _.uniqBy<IComment>([...comments, ...item.data.data], 'id')
                  setComments(uniqArr);
                } else {
                  setNoMore(true);
                }
        ...
        }),
        {
          ...
          onSuccess: (data) => {
            ...
            if (item.code === CUSTOM_STATUS_CODE.SUCCESS) {
              if (comments.length > 0) {
                if (item.data?.data.length !== 0) {
                  const uniqArr = _.uniqBy<IComment>([...item.data.data, ...comments], 'id')
                  setComments(uniqArr);
                } else {
                  setNoMore(true);
    ...
    const onEndReached = React.useCallback(async () => {
      const startIndex = (currentPage -1) * perPage;
          ...
          if (currentPage < endIndex && comments.length > 0 && noMore === false && !isLoading) {
    ...
    <Box width='100%' minHeight={50} bg='mainBackGroundColor' position='relative'>
        ...
        <Box flex={1} maxHeight={300} paddingTop='s' borderTopWidth={0.5} borderTopColor='lightGrey' marginHorizontal='m'>
            ...
            <FlatList 
                  ...
                  ListEmptyComponent={() => (
                    <>
                      { !isLoading || !paginationMutation.isLoading && (
                        <Box width={'100%'} height={50} justifyContent={'center'} alignItems={'center'}>
                            <CustomText variant='subheader' fontSize={14}>No comment. Be the first to comment on this post</CustomText>
                        </Box>
                      )}
                    </>
                  )}
        ...
        <Box width='100%' minHeight={90}  bg='mainBackGroundColor'  overflow='visible' elevation={4} borderTopWidth={0.3} borderTopColor='lightGrey' paddingVertical='s' paddingHorizontal='m'>
           { images.length > 0 && (
            ...
            <CommentTextBox onSubmit={() => handleSubmit()} isLoading={mutationLoading} text={text} onTextChange={(e) => handleTextChange(e)} onImagePicked={handleImagePicked} />
        </Box>

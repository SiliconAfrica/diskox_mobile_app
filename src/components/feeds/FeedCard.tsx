...
const FeedCard = ({
  ...
  return (
    <Box width="100%" bg={isDarkMode ? 'secondaryBackGroundColor' : 'mainBackGroundColor'} borderBottomWidth={0.3} borderTopWidth={0.3} borderBottomColor={isDarkMode ? 'secondaryBackGroundColor' : "lightGrey"} borderTopColor={isDarkMode ? 'secondaryBackGroundColor' : "lightGrey"} style={{ marginBottom: 15 }}>
      ...
      <Box flexDirection='row' alignItems='flex-start' width='100%' justifyContent='space-between' paddingHorizontal='s' paddingVertical='m'>
        ...
        <Box flexDirection='row' flex={0.8} width='100%'>
          ...
          <Box width={'100%'} marginLeft='s' overflow='hidden'>
              {activePost.reposted_post.length < 1 && (
              ...
              {activePost.reposted_post.length > 0 && (
                 <Box width={'100%'} flexWrap='wrap' flexDirection='row' alignItems='center'>
                 <CustomText marginRight='s' variant='subheader' fontSize={14} >
                  Reposted by {name} @{username}
                 </CustomText>
               </Box>
              )}
          ...
          <Box marginHorizontal='m'>
            ...
            <Box paddingHorizontal='s' flexDirection='row' width={'100%'} flexWrap='wrap' marginBottom='m'>
              {post?.post_type === 'question' && (
                <>
                  {showMoreTitle ? <CustomText variant='header' fontSize={16}>{post.repost_comment}</CustomText> : post.repost_comment?.length > 150 ? <CustomText variant='header' fontSize={16}>{`${post.repost_comment?.slice(0, 150)}...`}</CustomText> : <CustomText variant='header' fontSize={16}>{post.repost_comment}</CustomText>}
                  {post.repost_comment?.length > 150 && <CustomText style={{ width: '100%' }} onPress={() => setShowMoreTitle(!showMoreTitle)} color={showMoreTitle ? 'lightGrey' : 'primaryColor'}>{showMoreTitle ? 'Read less' : 'Read more'}</CustomText>}
                </>
              )}
              {post?.repost_comment !== null && (
            ...
            <RepostFeedCard post={activePost} followLoading={follow.isLoading} handleBookmark={handleBookmark} handleFollow={handleFollow} handleNavigate={handleNavigate} showReactions={showReactions} vote={(id) => vote(id)} />
      ...
      {
        activePost.reposted_post.length < 1 && (
          <PostDetails post={activePost} vote={(id) => vote(id)} />
        )
      }

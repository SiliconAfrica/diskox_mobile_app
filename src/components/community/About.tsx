...
const AboutCommunity = () => {
  ...
  const moderatorQuery = useQuery(
    ["getModerators", id],
    () => httpService.get(`${URLS.GET_COMMUNITY_MODERATORS}/${data.id}`),
    {
      onSuccess: (data) => {
        if (data?.data?.code === 3) {
      ...
      >
        ...
        <Box paddingBottom="m" marginTop="m">
          <Box flexDirection="row" alignItems="center" marginBottom="l">
            ...
            />
            <CustomText variant="body" fontSize={16} marginLeft="s">
              Created {moment(details?.created_at).toDate().toDateString()}
            </CustomText>
          </Box>
      ...
      >
        ...
        {!moderatorQuery.isLoading && moderatorQuery.isError && (
          ...
          >
            ...
            <PrimaryButton
              title="Retry"
              onPress={() => moderatorQuery.refetch()}
            />
          </Box>
        )}
        {!moderatorQuery.isLoading && !moderatorQuery.isError && (

...
import { passwordSchema, usernameSelectSchema } from '../../../services/validations'
...
const Password = () => {
  ...
  const { username, email, setAll: setValues } = useSignupState((state) => state);
  const { setAll: updateUser } = useDetailsState((state) => state)

  const { isLoading, mutate } = useMutation({
    mutationFn: (data: RegisterPayload) => httpService.post(`/auth/register`, data),
    onError: (error: any) => {
      alert(error.message);
    },
  ...
  return renderForm(
    <Box flex={1} marginTop='xl'>
      ...
      <CustomTextInput name='password' placeholder='Password' isPassword label='Password' containerStyle={{ marginTop: 20 }}  />
      <CustomTextInput name='password_confirmation' placeholder='Confirm Password' label='Confirm Password' isPassword  containerStyle={{ marginTop: 20 }}  />

      <Box height={20} />
      <SubmitButton label='Continue' onSubmit={navigate} isLoading={isLoading} />
    </Box>
  )
}

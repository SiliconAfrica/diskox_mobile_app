...
const Setup = ({ navigation  }: NativeStackScreenProps<RootStackParamList>) => {
  ...
  const renderStates = React.useCallback(() => {
    return (States as IState[]).filter((item) => item.country_id === selected.id).map((item) => item.name);
  }, [selected.id])
  ...
  return (
    <Box backgroundColor='mainBackGroundColor' flex={1} paddingTop='xl' paddingHorizontal='m'>
     ...
     <Box width='100%' height={100} justifyContent='center' alignItems='center'>
      ...
      <CustomPicker onPicked={(data) => setState(data)} data={renderStates()} label='State' />

      <GenderPicker onChange={(gen: string) => setGender(gen)} />

      <CustomText variant='body' mt='l'>Date of birth</CustomText>

      <Box width='100%' height={50} borderRadius={10} borderWidth={2} borderColor='secondaryBackGroundColor' >
        <Pressable 
        ...
        style={{
          ...
          <Feather name='chevron-down' size={25} color={theme.colors.textColor} />
        </Pressable>
      </Box>

...
const SettingCard = ({
  ...
  const { mutate, isLoading } = useMutation({
    mutationFn: (data: any) =>
      httpService.put(`${URLS.UPDATE_NOTIFICATION_SETTINGS}`, data),
    onSuccess: (data) => {
    ...
    >
      ...
      {!isLoading && (
        <Switch
          ...
          offColor={theme.colors.secondaryBackGroundColor}
          onValueChange={(val) => mutate({ notification: title })}
        />
      )}
      {isLoading && (
...
const NotificaitionsSettings = () => {
  ...
  const { isLoading, isError, refetch } = useQuery(
    ...
    () => httpService.get(`${URLS.GET_NOTIFICATIONS_SETTINGS}`),
    {
      onSuccess: (data) => {
        ...
        <Box marginTop="l" width="100%" paddingHorizontal="m">
          {notififcations.map((notif, index) => {
            return (
              <SettingCard
                title={notif}
                isChecked={states[notif] === 1 ? true : false}
                key={index.toString()}
              />
            );
          })}

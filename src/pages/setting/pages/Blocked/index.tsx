...
const BlockedUserCard = ({ username, name, profile_image }: IBlockUser) => {
  ...
  const { isLoading, isError } = useMutation({
    mutationFn: (data: string) =>
      httpService.post(`${URLS.BLOCK_AND_UNBLOCK_USER}/${data}`),
    onSuccess: (data) => {
...
const BlockedUsers = () => {
  // const theNavi =
  //   useNavigation<
  //     NativeStackScreenProps<RootStackParamList, "notifications-settings">
  //   >();
  // const navigation = theNavi.navigation;
  ...
  const { isLoading, refetch } = useQuery(
    ...
    () =>
      httpService.get(`${URLS.FETCH_BLOCKED_USERS}`, {
        params: {
          page,
        },
      }),

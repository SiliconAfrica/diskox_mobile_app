...
const ResetPassword = ({
  navigation,
}: NativeStackScreenProps<RootStackParamList, "reset-password">) => {
  const { stage, setAll } = useResetState((state) => state);
  ...
  return (
    <Box flex={1} backgroundColor="mainBackGroundColor" paddingHorizontal="m">
      <Box flexDirection="row" alignItems="center" paddingTop="l" flex={0.13}>
        ...
        />
        <CustomText variant="subheader" marginLeft="s" fontSize={23}>
          Reset Password
        </CustomText>
      </Box>
      {switchPages()}

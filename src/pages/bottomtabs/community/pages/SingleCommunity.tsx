...
const SingleCommunity = () => {
    ...
    const route = useRoute<RouteProp<CommunityStackParamList, "community-username">>();
    ...
    const { isLoading: isLoadingRole } = useQuery(
        ["getCommunityRole", details?.id],
        () => httpService.get(`${URLS.CHECK_COMMUNITY_ROLE}/${details?.id}`),
        {
    ...
    const { isLoading: isJoining, mutate } = useMutation({
        mutationFn: () => httpService.post(`${URLS.JOIN_COMMUNITY}/${details?.id}`),
        onSuccess: (data) => {
            toast.show(`${data?.data?.message || "Success"}`, {
                type: "success",
            });
            queryClient.invalidateQueries(["getCommunities"]);
            queryClient.invalidateQueries(["getCommunity", details?.id]);
        },
        onError: (e) => {
    ...
    const { isLoading: isAccepting, mutate: acceptInvite } = useMutation({
        mutationFn: () =>
            httpService.post(`${URLS.ACCEPT_COMMUNITY_INVITATION}/${details?.id}`),
        onSuccess: (data) => {
    ...
    const { isLoading: isDeclining, mutate: declineInvite } = useMutation({
        mutationFn: () =>
            ...
            httpService.post(`${URLS.DECLINE_COMMUNITY_INVITATION}/${details?.id}`),
        onSuccess: (data) => {
    ...
    const { isLoading: isUploading, mutate: changeBanner } = useMutation({
        mutationFn: (body: FormData) =>
            httpService.post(`${URLS.UPDATE_COMMUNITY_BANNER_IMAGE}/${details?.id}`, body),
        onSuccess: (data) => {
    ...
    const { mutate: changeProfileImage } = useMutation({
        mutationFn: (body: FormData) =>
            httpService.post(`${URLS.UPDATE_COMMUNITY_PROFILE_IMAGE}/${details?.id}`, body),
        onSuccess: (data) => {
            toast.show(data?.data?.message || "Updated successfully", {
                type: "success",
            });
...
export default memo(SingleCommunity);

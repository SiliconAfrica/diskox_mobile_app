...
const UserPosts = ({ id }: IProps) => {
    ...
    const getPosts = useQuery(['getProfileCommentsPosts', id, currentPage], () => httpService.get(`${URLS.GET_PROFILE_COMMENTS}/${id}`, {
        params: {
            page: currentPage,
        }
    }), {
...

       {
           !getPosts.isLoading && posts.length < 1 && (
               <Box justifyContent='center' alignItems='center' height={50} paddingTop='l'>
                <CustomText variant='subheader' fontSize={18} color='primaryColor'>No Comments</CustomText>
               </Box>
           )
       }
